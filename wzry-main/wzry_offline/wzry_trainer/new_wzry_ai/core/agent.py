...
class DoubleDQN:
    def __init__(self,action_dims: Tuple[int, int] = (9, 8)):
        ...
        # 初始化 TensorBoard 记录器
        self.writer = SummaryWriter(log_dir=TrainingConfig.LOSS_IMAGE_DIR)
        # **记录模型结构**
        ...
        self.writer.add_graph(self.current_network, [dummy_input1, dummy_input2, dummy_state])


    def learn(self) -> Optional[Tuple[float, float]]:

        with (self.train_lock):
            if memory.memory_buffer_len() < TrainingConfig.BATCH_SIZE:
                print_utils.print_green("None")
                return None

            # 采样数据

            image1s, image2s, states_vectors,actions, rewards, next_image1s,next_image2s,next_states_vectors, dones, indices, weights = memory.sample(TrainingConfig.BATCH_SIZE)
            ...
            # 记录到 TensorBoard（每 100 步记录一次）
            if self.state.steps_done % 1000 == 0:
                self.writer.add_scalar("Loss/Left", loss_left.item(), self.state.steps_done)
                self.writer.add_scalar("Loss/Right", loss_right.item(), self.state.steps_done)
                self.writer.add_scalar("Q-Value/Left", current_q_left.mean().item(), self.state.steps_done)
                self.writer.add_scalar("Q-Value/Right", current_q_right.mean().item(), self.state.steps_done)
                self.writer.add_scalar("Epsilon", self.state.epsilon, self.state.steps_done)
                self.writer.add_scalar("Learning Rate", self.optimizer.param_groups[0]['lr'], self.state.steps_done)

            # 记录梯度范数和参数分布（每 1000 步记录一次）
            if self.state.steps_done % 10000 == 0:
                total_norm = 0
                for name, param in self.current_network.named_parameters():
                    if param.grad is not None:
                        param_norm = param.grad.norm().item()
                        total_norm += param_norm ** 2
                        self.writer.add_scalar(f"Gradients/{name}", param_norm, self.state.steps_done)
                total_norm = total_norm ** 0.5
                self.writer.add_scalar("Gradients/Total Norm", total_norm, self.state.steps_done)

                # 记录网络参数分布
                for name, param in self.current_network.named_parameters():
                    if param.numel() > 0:
                        self.writer.add_histogram(f"Parameters/{name}", param, self.state.steps_done)

                self.writer.flush()  # 控制日志写入频率
            ...
            if self.state.steps_done % 100000 == 0:
                self.save_model(TrainingConfig.MODEL_PATH)
