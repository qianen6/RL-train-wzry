...
class TrainingConfig:
    """训练相关配置"""
    USE_TENSORRT=True  #是否使用TensorRT进行推理加速，True为使用
    ...
    ONNX_MODEL_PATH='../models/wzry_model.onnx'   #onnx模型路径
...
class GameConfig:
    ...
    action_mode: Final[str] = "agent"  #player为玩家自己，agent为智能体

@dataclass(frozen=False)
class Reward_Config:
    """奖励相关配置"""
    # 移动奖励
    MOVE_TO_TARGET = 0  # 向目标移动奖励
    MOVE_PENALTY = 0  # 远离目标惩罚
    MOVE_To_Edge=-10  #边界卡住惩罚
    ATTACKED_BY_TOWER = -10
    ...
    GAME_VICTORY = 10000.0  # 游戏胜利奖励
    GAME_DEFEAT = -10000.0  # 游戏失败惩罚
    ...
    def get_healing_reward(self, self_blood: float, blood_healed: float):
        return 10 * (1 - self_blood) * blood_healed

    def get_attacked_reward(self, self_blood: float, blood_attacked: float):
        if self_blood == 1.0 and blood_attacked == -1.0:
            return -5
        return -10 * (1 - self_blood) * (-blood_attacked)
...
other_status:dict={
    ...
    "move_target":(282,206),#移动目标，初始值为我方发育路一塔
