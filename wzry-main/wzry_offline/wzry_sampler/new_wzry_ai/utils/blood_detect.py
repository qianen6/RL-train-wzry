...
class BloodDetector:
    def __init__(self, config: BloodDetectorConfig = BloodDetectorConfig()):
        ...
        self.config = config
        self.blood_healed = 0.0
        self.enemy_blood_changed = False
        self.last_check_enemy_blood_time = 0
        self.last_check_self_blood_time = 0
        self.last_enemy_blood = 0.0
        self.last_self_blood = 0.0
    ...
    def is_enemy_blood_changed(self, image: np.ndarray) -> bool:
        ...
        if abs(current_blood - self.last_enemy_blood) > 0.01:
            ...
            self.last_check_enemy_blood_time = current_time
        elif current_time - self.last_check_enemy_blood_time > self.config.change_duration:
            self.enemy_blood_changed = False
        self.last_enemy_blood = current_blood
        return self.enemy_blood_changed


    def self_blood_healed(self, image: np.ndarray) -> float:
        current_blood = self.get_self_blood(image)
        current_time = time.time()
        ...
        if current_blood == 0.0:
            self.last_self_blood = 0.0
            self.blood_healed = 0.0
            return self.last_self_blood, self.blood_healed

        # 如果是第一次检测到血量，更新记录但返回False
        if self.last_self_blood == 0.0:
            self.last_self_blood = current_blood
            return self.last_self_blood, 0.0

        # 检查血量是否发生变化（使用阈值避免微小波动）
        blood_healed = current_blood - self.last_self_blood
        if abs(blood_healed) > 0.05:
            self.blood_healed = blood_healed
            self.last_check_self_blood_time = current_time
        elif current_time - self.last_check_enemy_blood_time > self.config.change_duration:
            self.blood_healed = 0.0
        last_self_blood = self.last_self_blood
        self.last_self_blood = current_blood
        return last_self_blood ,self.blood_healed