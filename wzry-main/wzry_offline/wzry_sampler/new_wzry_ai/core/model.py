...
class TemporalStateEncoder(nn.Module):
    ...
    def forward(self, x,h0=None):
        ...
        if h0 is None:
            batch_size = x.size(0)
            h0 = torch.zeros(2, batch_size, self.gru.hidden_size).to(x.device)
...
class SpatiotemporalEncoder(nn.Module):
    ...
    def forward(self, x):
        x=self.conv3d(x)
        # 使用插值替代自适应池化
        x = F.interpolate(x, size=(4, 36, 20), mode='trilinear', align_corners=False)
        return x
...
class HybridNet(nn.Module):
    ...
    def forward(self, image1, image2, states):
        ...
        # 使用插值替代自适应池化
        minimap_feat = F.interpolate(minimap_feat, size=(4, 2), mode='bilinear', align_corners=False)  # [B, 128, 4, 2]
        minimap_feat = minimap_feat.contiguous().view(minimap_feat.size(0), -1)  # [B, 128*4*2]
        minimap_feat = self.minimap_proj(minimap_feat)  # 降维到 [B,128]

        state_feat = self.state_encoder(states,None)
