...
class WzryTrainer:
    def __init__(self, config: TrainerConfig = TrainerConfig()):
        ...
        self.action_mode = GameConfig.action_mode  # "agent" 或 "player"  按下l为agent 按下m为player
        ...
        self.attack_key_queue = queue.Queue()  # 攻击键事件队列
        self.attack_key_states = {}  # 记录每个键的状态和时间戳 {key: {'state': 'pressed/released', 'timestamp': float}}
        self.min_duration = 0.05  # 最小持续时间（秒），可根据需要调整，例如 0.1秒
        self.max_hold_duration = 1.0  # 最大保持时间（秒），超过此时间自动释放
        ...
        self.experience_threshold = 20  # 每300个经验发送一次
        self.server_url = "http://192.168.31.6:5000/transport/receive_experiences"  # 目标主机的地址

        # 启动一个后台线程来检查经验批次的大小
        # self.experience_check_thread = threading.Thread(target=self.check_experience_batch_size)
        # self.experience_check_thread.daemon = True  # 设置为守护线程，主线程退出时自动退出
        # self.experience_check_thread.start()

        self.first_training = True  # 用于跟踪是否是第一次训练


    def save_experiences(self):
        print_utils.print_green("strat to save")
        folder_path = "../experiences"
        os.makedirs(folder_path, exist_ok=True)
        existing_files = os.listdir(folder_path)
        file_index = len(existing_files)
        save_path = f"../experiences/experience_{file_index + 1}.pth"
        torch.save(self.experience_batch, save_path)
        self.experience_batch.clear()  # 清空经验批次，重新开始收集
        print_utils.print_green("Experience batch saved to local")

    def check_experience_batch_size(self):
        while not self.stop_event.is_set():  # 如果没有停止事件，继续检查
            time.sleep(1)  # 每隔1秒检查一次
            if len(self.experience_batch) >= self.experience_threshold:
                folder_path = "../experiences"
                os.makedirs(folder_path, exist_ok=True)
                existing_files = os.listdir(folder_path)
                file_index = len(existing_files)
                save_path = f"../experiences/experience_{file_index + 1}.pth"
                torch.save(self.experience_batch, save_path)
                #self.send_experience(self.experience_batch)  # 执行经验发送
                self.experience_batch.clear()  # 清空经验批次，重新开始收集
                #print("Experience batch sent and cleared.")
                print(f"Experience batch saved to: {save_path}")
                print_utils.print_green("Experience batch saved to:")

    def send_experience(self, experience_batch):
        try:
            # 记录开始时间：经验批次转换为JSON格式
            start_conversion_time = time.time()
            # 将经验批次转换为JSON格式
            data = json.dumps(experience_batch, default=self.serialize_experience)

            # 打印数据大小（字节数）
            data_size = len(data.encode('utf-8'))  # 使用utf-8编码后计算字节数
            print(f"数据大小: {data_size} 字节")

            print("serialize_experience successfully--------------------")
            # 记录转换完成的时间
            end_conversion_time = time.time()
            conversion_time = end_conversion_time - start_conversion_time
            print(f"经验批次转换时间: {conversion_time}秒")
            ...
            print(f"发送请求时间: {sending_time}秒")

            start_respond_time = time.time()
            if response.status_code == 200:
                print("Experience batch successfully sent.")
                end_respond_time = time.time()
                respond_time = start_respond_time - end_respond_time
                print(f"接收回应时间: {respond_time}秒")
            else:
                print(f"Failed to send experience batch. Status code: {response.status_code}")
        except Exception as e:
            print(f"Error while sending experience batch: {e}")

    def serialize_experience(self, obj):
        """自定义序列化方法，将numpy数组转化为列表，处理state中包含的列表结构"""
        if isinstance(obj, np.ndarray):
            return obj.tolist()  # 将numpy数组转化为列表
        elif isinstance(obj, tuple):
            return list(obj)  # 将元组转化为列表
        elif isinstance(obj, list):
            # 递归处理列表中的每个元素
            return [self.serialize_experience(item) for item in obj]
        elif isinstance(obj, dict):
            return {key: self.serialize_experience(value) for key, value in obj.items()}  # 如果是字典
    ...
    def on_press(self, key):
        ...
        # 检查是否是攻击按键
            ...
            self.attack_key_queue.put(('press', key_name, time.time()))
    ...
    def on_release(self, key):
        ...
        if key_name in self.key_pressed_attack:
            ...
            self.attack_key_queue.put(('release', key_name, time.time()))
    ...
    def _get_player_action(self):
        ...
        # 1. 处理队列事件（直接更新状态）
        while not self.attack_key_queue.empty():
            event_type, key, timestamp = self.attack_key_queue.get()
            if event_type == 'press':
                # 记录/覆盖按键状态
                self.attack_key_states[key] = {
                    'press_time': timestamp,
                    'released': False
                }
            elif event_type == 'release':
                # 直接移除按键状态（无论持续时间长短）
                if key in self.attack_key_states:
                    del self.attack_key_states[key]

        # 2. 定义明确的优先级顺序
        priority_order = ['f', 'c', 'b', 'r', 'e', 't', 'j']

        # 3. 收集有效按键（满足最小持续时间且未超时）
        valid_keys = []
        for key in list(self.attack_key_states.keys()):
            state = self.attack_key_states[key]
            duration = current_time - state['press_time']

            # 检查超时（超过max_hold_duration自动失效）
            if duration > self.max_hold_duration:
                del self.attack_key_states[key]
                continue
    ...
    def train_episode(self , episodes: int) -> Tuple[float, int, Optional[bool]]:
        ...
        while not self.episode_stop_event.is_set():
            ...
            if self.action_mode == "agent":
                ...
                print(f"推理时间: {infer_end_time - infer_start_time:.6f} 秒")
            ...
            send_action = [left_action, right_action]
            try:
                # 执行动作
                action_start_time = time.time()
                next_state, reward, done, info = self.env.step(action)
                action_end_time = time.time()
                print(f"step时间: {action_end_time - action_start_time:.6f} 秒")
            except GameNotReadyError as e:
                self.logger.error("The game environment is not ready", exc_info=e)
                break
            ...
            # 新增代码 将当前经验加入经验批次
            experience = [state, send_action, reward, next_state, done]
            ...
            # 等待检测结果
            game_continue, game_result =self.check_game_status(self.env.state.current_frame)
