...
class ActionExecutor:
    ...
    def _execute_hand_action(self, action: int, is_left: bool) -> None:
        actions_map = (self.constants.LEFT_ACTIONS if is_left
                      else self.constants.RIGHT_ACTIONS)

        if action not in actions_map:
            raise InvalidActionError(f"无效的动作ID: {action}")

        hand_action = actions_map[action]
        if not hand_action.keys:
            return
    ...
    def execute_action_async(self, left_action: int, right_action: int) -> None:
        ...
        # 等待两个动作都执行完成
        done, not_done = wait([left_future, right_future])
        # 检查是否有异常
        for future in done:
            if future.exception():
