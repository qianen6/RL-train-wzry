...
class TowerAttackDetector:
    ...
    def is_red_line_through_point(self, red_mask, point, visual_debug=False):
        ...
        :return: (bool, 可视化图像)
        """
        #cv2.imshow("dddddddddda", red_mask)
        # 形态学优化（连接断裂区域）
    ...
    def detect(self, frame: np.ndarray) -> bool:
        ...
        if not current_status:
            current_status, debug = self.is_red_line_through_point(red_mask, (800, 360), visual_debug=True)
        ...
        if not self.history:
            return False
        return sum(self.history) / len(self.history) >= self.confidence
