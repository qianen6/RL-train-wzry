...
from new_wzry_ai.core.model import DQN,HybridNet
...
class DoubleDQN:
    def __init__(self, 
                 state_dim: Tuple[int, int, int] = (4, 144, 256),
                 action_dims: Tuple[int, int] = (9, 8)):
    ...
    def select_action(self, state: np.ndarray) -> Tuple[int, int]:
        with self.action_lock:
            if np.random.rand() < self.state.epsilon:
                ...
                image1 = torch.tensor(np.array(image1), dtype=torch.float32).unsqueeze(0).to(self.device)
                image2 = torch.tensor(np.array(image2), dtype=torch.float32).unsqueeze(0).to(self.device)
                state_vector = torch.tensor(np.array(state_vector), dtype=torch.float32).unsqueeze(0).to(self.device)
                with torch.no_grad():
                    left_q_values,right_q_values = self.current_network(image1,image2,state_vector)# 获取当前网络的输出
                left_action = left_q_values.argmax().item()
                right_action = right_q_values.argmax().item()
